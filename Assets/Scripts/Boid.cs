...
public class Boid : MonoBehaviour
{
    ...
    //Close to finish prod, Speed based how close to target boid is 
    public Vector3 Pursue(Vector3 target, float magnitude)
    {
        ...
        //Find the vector goes to target from my pos, (the difference from pos to target) 

        Vector3 toTarget = target - pos;

        Vector3 desiredVel = toTarget.normalized * maxSpeed;

        Vector3 deltaVel = desiredVel - velocity;

        //Vector3 acceleration = desiredVel.normalized * maxXLR; WRONG!!!
        Vector3 acceleration = deltaVel.normalized * maxXLR;

        return acceleration;
    }

    public Vector3 Arrive(Vector3 target, float magnitude)
    {
        // Pursure 2 lol
        Vector3 velocity = rigidBody.linearVelocity;
        ...
        {
            float distancePercent = range * toTarget.magnitude;
            acceleration = acceleration * distancePercent;
        }
